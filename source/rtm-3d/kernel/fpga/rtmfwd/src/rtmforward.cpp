...
#if RTM_numFSMs == 1
    s[0].forward(p_src[0], l_vt_in, l_vt_out, l_p[0], l_p[1]);
#else
...
#pragma HLS RESOURCE variable = l_vt core = fifo_uram
    s[0].forward(p_src[0], l_vt_in, l_vt[0], l_p[0], l_p[1]);
    for (int i = 1; i < RTM_numFSMs - 1; i++) {
#pragma HLS UNROLL
        s[i].forward(p_src[i], l_vt[i - 1], l_vt[i], l_p[i], l_p[i + 1]);
    }
    s[RTM_numFSMs - 1].forward(p_src[RTM_numFSMs - 1], l_vt[RTM_numFSMs - 2], l_vt_out, l_p[RTM_numFSMs - 1],
                               l_p[RTM_numFSMs]);
#endif
...
#pragma HLS ARRAY_PARTITION variable = l_src complete dim = 1
    ...
    for (int t = 0; t < p_t / RTM_numFSMs; t++) {
        ...
        forward(l_s, t, l_src, p_v2dt2, p_p0, p_p1);
    }
}
